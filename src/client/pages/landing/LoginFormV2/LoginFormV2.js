...
const LoginFormV2 = ({ isLoginForm }) => {
  const { loading, success, login } = useContext(AuthContext)
  ...
  return (
    <Wrapper disabled={loading || success} isLoginForm={isLoginForm}>
      <Header>Log in with</Header>
      <OAuthGroup>
        <OAuthButton>
          <Google size="28" color="#f14236" />
          Google
        </OAuthButton>
        <OAuthButton>
          <Facebook size="28" color="#4267b2" />
          Facebook
        </OAuthButton>
      </OAuthGroup>
      <OrLabel>or</OrLabel>
